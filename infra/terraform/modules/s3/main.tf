...
# Apache Iceberg warehouse — Glue 5.0 writes silver.cdc_events here in Iceberg
# format, queryable via Athena / Trino / EMR without needing to export data.
resource "aws_s3_bucket" "iceberg" {
  bucket = "${var.project_prefix}-iceberg-${var.environment}-${random_string.suffix.result}"
}
